...
fnorm       = ['0x00800001', '0x80800001', '0x00855555', '0x80855555', '0x008AAAAA', '0x808AAAAA', '0x55000000', '0xD5000000', '0x2A000000', '0xAA000000']# ***single,double and quad don't match
...
dsubnorm    = ['0x0000000000000002', '0x8000000000000002','0x0008000000000000', '0x0008000000000002', '0x0001000000000000', '0x8001000000000000','0x8001000000000003','0x8001000000000007'] # ***missing 2nd largest and walking subnorms
...
qminsubnorm = ['0x00000000000000000000000000000001', '0x80000000000000000000000000000001']
qsubnorm    = ['0x00000000000000000000000000000002','0x80000000000000000000000000000002','0x0000FFFFFFFFFFFFFFFFFFFFFFFFFFFE','0x8000FFFFFFFFFFFFFFFFFFFFFFFFFFFE','0x00008000000000000000000000000000', '0x00008000000000000000000000000002', '0x00001000000000000000000000000000', '0x80001000000000000000000000000000','0x80001000000000000000000000000003','0x80001000000000000000000000000007']
qmaxsubnorm = ['0x0000FFFFFFFFFFFFFFFFFFFFFFFFFFFF', '0x8000FFFFFFFFFFFFFFFFFFFFFFFFFFFF']
qminnorm    = ['0x00010000000000000000000000000000', '0x80010000000000000000000000000000']
qnorm       = ['0x00010000000000000000000000000002', '0x80010000000000000000000000000002','0x00011000000000000000000000000000', '0x80011000000000000000000000000000', '0x00018000000000000000000000000000', '0x80018000000000000000000000000000','0x80018000000000000000000000000005','0x80018000000000000000000000000007']
qmaxnorm    = ['0x7FFEFFFFFFFFFFFFFFFFFFFFFFFFFFFF', '0xFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFF']
qinfinity   = ['0x7FFF0000000000000000000000000000', '0xFFFF0000000000000000000000000000']
qdefaultnan = ['0x7FFF8000000000000000000000000000', '0xFFFF8000000000000000000000000000']
qqnan       = ['0x7FFF8000000000000000000000000001', '0xFFFF8000000000000000000000000001']
qsnan       = ['0x7FFF0000000000000000000000000001', '0xFFFF0000000000000000000000000001', '0x7FFF4AAAAAAAAAAAAAAAAAAAAAAAAAA', '0xFFFF4AAAAAAAAAAAAAAAAAAAAAAAAAAA']
qone        = ['0x3FFF0000000000000000000000000000', '0xBFFF0000000000000000000000000000']
...
get_sanitise_func = lambda opcode: sanitise_norm if any([x in opcode for x in \
        ['fsgnj','fle','flt','feq','fclass','flw','fsw','fld','fsd','flq','fsq','fmin','fmax']]) else \
...
def num_explain(flen,num):
    num_dict = {
        ...
        tuple(done)         : 'done',
        tuple(qzero)         : 'qzero',
        tuple(qminsubnorm)     : 'qminsubnorm',
        tuple(qsubnorm)     : 'qsubnorm',
        tuple(qmaxsubnorm)     : 'qmaxsubnorm',
        tuple(qminnorm)     : 'qminnorm',
        tuple(qnorm)         : 'qnorm',
        tuple(qmaxnorm)     : 'qmaxnorm',
        tuple(qinfinity)     : 'qinfinity',
        tuple(qdefaultnan)     : 'qdefaultnan',
        tuple(qqnan)         : 'qqnan',
        tuple(qsnan)         : 'qsnan',
        tuple(qone)         : 'qone'
    }
    ...
    if(int(exp,2)!=0):
        return('fnorm' if flen==32 else 'dnorm' if flen == 64 else 'qnorm')
    else:
        return('fsubnorm' if flen==32 else 'dsubnorm' if flen == 64 else 'qsubnorm')
...
def fields_dec_converter(flen, hexstr):                            # IEEE-754 Hex -> Decimal Converter
    ...
    if(flen == 32):
        ...
        #print(int(exp,2))
        if (int(exp, 2) - 16383) < -16382:
            conv_num = 0.0
            exp_str += str(-16382) + ')'
        elif (int(exp, 2) - 16383) >= -16382:
            conv_num = 1.0
            exp_str += str(int(exp, 2)-16383) + ')'
...
def floatingPoint_tohex(flen,float_no):                            # Decimal -> IEEE-754 Hex Converter
    ...
    if(flen==32):
        ...
        check_sn = nor.split("p")[0].split(".")[0]
        if(int(check_sn[len(check_sn)-1])==0):                    # Checking Underflow of Exponent
            exp_bin=('0'*15)                            # Exponent of Subnormal numbers
            exp_sn=int(nor.split("p")[1])
            num="SN"
        elif(int(nor.split("p")[1])>16383):                        # Checking Overflow of Exponent
            if(sign==0):
                return "0x7FFEFFFFFFFFFFFFFFFFFFFFFFFFFFFF"                    # Most Positive Value
            else:
                return "0xFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFF"                    # Most Negative Value
        else:                                        # Converting Exponent to 8-Bit Binary
            exp=int(nor.split("p")[1])+16383
            exp_bin=('0'*(15-(len(bin(exp))-2)))+bin(exp)[2:]
    ...
    if(flen==32):
        ...
        if(num == "SN"):
            mant_bin=bin(int('1'+mant[2:],16))[3:]
        binary="0b"
        binary=binary+str(sign)+exp_bin+mant_bin[0:111]
        hex_tp=hex(int(binary,2))
        hex_tp=hex_tp.replace('0x','0x'+'0'*(32-(len(hex_tp)-2)))
...
def ibm_b1(flen, iflen, opcode, ops):
    ...
    if iflen == 32:
        ...
        basic_types = qzero + qminsubnorm + [qsubnorm[0], qsubnorm[1]] + \
                      qmaxsubnorm + qminnorm + [qnorm[0], qnorm[1]] + qmaxnorm + \
...
#            cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
        ...
        if opcode.split('.')[0] in ["fadd","fsub","fmul","fdiv","fsqrt","fmadd","fnmadd","fmsub","fnmsub","fcvt","fmv"]:
            ...
            ["fclass","flt","fmax","fsgnjn","fmin","fsgnj","feq","flw","fsw","fsgnjx","fld","fle","fsd","flq","fsq"]: #***missing fsd and flq,fsq
            cvpt = sanitise(0,cvpt,iflen,flen,ops)
        else:
    ...
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B1 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
    return coverpoints

def ibm_b2(flen, iflen, opcode, ops, int_val = 100, seed = -1): #***Quad support isn't possible due to python limitations, come back later : fpcalc/quadmath/lookuptable
    '''
    ...
    if iflen == 32:
        ...
        flip_types = qzero + qone + qminsubnorm + qmaxsubnorm + qminnorm + qmaxnorm
...
#            cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B2 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
    #print(rs1)
    #return coverpoints
...
def ibm_b3(flen,iflen, opcode, ops, seed=-1):
    ...
    if iflen == 32:
        ...
        maxdec = '1.7976931348623157e+308' # not used in the program
        ieee754_maxnorm_quad = '0x1.fffffffffffffffffffffffffffffp+16383'
        significand = ieee754_maxnorm_quad.split('p')[0][2:]  # Remove the "0x" prefix
        exponent = int(ieee754_maxnorm_quad.split('p')[1], 16)
        int_part, frac_part = significand.split('.')
        int_value = int(int_part, 16)
        frac_value = Decimal(int(frac_part, 16)) / Decimal(16 ** len(frac_part))

        # Combine the integer and fractional parts
        decimal_significand = Decimal(int_value) + frac_value

        # Construct the maximum quad precision value
        maxnum = decimal_significand * Decimal(2) **Decimal(exponent)
        ieee754_num = []
        lsb = []
        for i in qsubnorm+qnorm:
            if int(i[-1],16)%2 == 1:
                lsb.append('1')
                lsb.append('1')
            else:
                lsb.append('0')
                lsb.append('0')
            float_val = str(fields_dec_converter(128,i))
            if float_val[0] != '-':
                ieee754_num.append(float_val)
                ieee754_num.append('-'+float_val)
            else:
        ...
        for k in range(len(ieee754_num)):
            for i in range(2, 16, 2):
                ...
                # Check if the 'e' character exists in the string
                if 'e' in ieee754_num[k]:
                    significand = ieee754_num[k].split('e')[0]
                    exponent = ieee754_num[k].split('e')[1]
                    ir_value = str(Decimal(significand) + Decimal(pow(i * 16, -14))) + 'e' + exponent
                else:
                    # If 'e' is not present, assume the entire string represents the significand
                    significand = ieee754_num[k]
                    ir_value = str(Decimal(significand) + Decimal(pow(i * 16, -14)))

                ir_dataset.append([ir_value, ' | Guard = ' + grs[0] + ' Sticky = ' + grs[2] + ' Sign = ' + sign + ' LSB = ' + lsb[k]])
        
    b4_comb = []

    for i in range(len(ir_dataset)):
        rs1 = Decimal(str(random.uniform(1, float(maxnum))))
        rs3 = Decimal(str(random.uniform(1, float(maxnum))))
        if opcode in 'fadd':
            if iflen == 32:
                rs2 = ir_dataset[i][0] - rs1
            elif iflen == 64 or iflen == 128:
...
#                        cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64) if iflen ==64 else str(128)) + '-bit coverpoints using Model B3 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)

    return coverpoints

def ibm_b4(flen, iflen, opcode, ops, seed=-1): #***overflow , write individual computational functions to prevent overflow -- check for test-breaking
    '''
...
def ibm_b5(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b6(flen, iflen, opcode, ops, seed=-1):#***underflow. 
    '''
    ...
    if iflen == 32:
        ...
        ir_dataset = []
        ieee754_minsubnorm_n = '-0x0.0000000000000000000000000001p-16382'
        minnum = float.fromhex(ieee754_minsubnorm_n)
        r=str("{:.2e}".format(random.uniform(minnum,minnum/2)))
        for i in range(2,16,2):
            grs = '{:04b}'.format(i)
...
#                        cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B6 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)

    return coverpoints

def ibm_b7(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b8(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
#                    cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64)) if iflen == 64 else str(128) + '-bit coverpoints using Model B9 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)

    return coverpoints

def ibm_b10(flen, iflen, opcode, ops, N=-1, seed=-1):#***overflow
    '''
...
def ibm_b11(flen, iflen, opcode, ops, N=-1, seed=-1):#***overflow
    '''
    ...
    if ops == 2:
        ...
        for i in range(len(rs1)):
            ...
            elif iflen == 128: rs2_exp = '{:015b}'.format(rs2_exp)
            for j in range(len(rs1_man)):
                rs2_sgn = rs1_sgn
                rs2_man = '0'*j + rs1_man[j:]                        # Leading 0s
...
#                    cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B11 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)

    return coverpoints

def ibm_b12(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b13(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b14(flen, iflen, opcode, ops, N=-1, seed=-1):#***overflow
    '''
...
def ibm_b15(flen, iflen, opcode, ops, N=-1, seed=-1):#***overflow
    '''
...
def ibm_b16(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b17(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b18(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b19(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b20(flen, iflen, opcode, ops, seed=-1):#***overflow
    '''
...
def ibm_b21(flen, iflen, opcode, ops):
    ...
    if iflen == 32:
        ...
        basic_types = qzero + qsubnorm + qnorm +\
            qinfinity + qdefaultnan + [qqnan[0], qqnan[1]] + \
            [qsnan[0], qsnan[1]]
    else:
        logger.error('Invalid iflen value!')
...
#            cvpt += 'rs'+str(x)+'_val=='+str(c[x-1]) # uncomment this if you want rs1_val instead of individual fields
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+ \
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B21 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)

    return coverpoints

def ibm_b22(flen, iflen, opcode, ops, seed=10):#***overflow
    '''
...
def ibm_b23(flen, iflen, opcode, ops):
    ...
    if iflen == 32:
        ...
        maxnum = 0x40fc0000000000000000000000000000
    ...
    for c in dataset:
        for rm in range(0,5):
            cvpt = ""
            for x in range(1, ops+1):
                cvpt += (extract_fields(iflen,c[0],str(x)))
                cvpt += " and "
            # cvpt += 'rm_val == '
            ...
            for y in range(1, ops+1):
                cvpt += 'rs'+str(y)+'_val=='
                cvpt += num_explain(iflen, c[0]) + '(' + str(c[0]) + ')'
                if(y != ops):
                    cvpt += " and "
            cvpt += " "+c[1]
            coverpoints.append(cvpt)
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B23 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b24(flen,iflen, opcode, ops):
    ...
    for data in dataset:
        float_no = float(data[0])
        hex_value = floatingPoint_tohex(iflen, float_no)
        b24_comb.append((hex_value, data[1]))
        #t = "{:e}".format(data[0])
    ...
    for c in b24_comb:
        for rm in range(0,5):
            ...
            for x in range(1, ops+1):
                ...
                cvpt += " and "
            # cvpt += 'rm_val == '
            if "fmv" in opcode or opcode in "fcvt.d.s" or opcode in "fcvt.q.d" or opcode in "fcvt.q.s":
                cvpt = sanitise(0,cvpt,iflen,flen,ops)
                # cvpt += '0'
            else:
                cvpt = sanitise(rm,cvpt,iflen,flen,ops)
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B24 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b25(flen, iflen, opcode, ops, seed=10):
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B25 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b26(xlen, opcode, ops, seed=10):
    ...
    for c in dataset:
        for rm in range(0,5):
            ...
            for x in range(1, ops+1):
                ...
                cvpt += " and "
            # cvpt += 'rm_val == '
            if "fmv" in opcode or opcode in "fcvt.d.wu" or opcode in "fcvt.q.wu":
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if xlen == 32 else str(64) if xlen == 64 else str(128)) + '-bit coverpoints using Model B26 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b27(flen, iflen, opcode, ops, seed=10):
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B27 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b28(flen, iflen, opcode, ops, seed=10):
    ...
    if iflen == 32:
        ...
        dataset.append((qzero[0],"+0"))
        dataset.append((floatingPoint_tohex(128,float(random.uniform(0,1))),"A random number in the range (+0, +1)"))
        dataset.append((qone[0],"+1"))
        for i in range(125,300,25):
            dataset.append((floatingPoint_tohex(128, i/100),"Number = "+str(i/100)+" => Number ∈ (1,2.75]"))
        dataset.append((floatingPoint_tohex(128,float(random.uniform(1,2**127-1))),"A random number in the range (+1, +1.11..11*2^precision)"))
        dataset.append((floatingPoint_tohex(128,float(2**127-1)),"MaxInt"))
        dataset.append((qinfinity[0],"+Infinity"))

        dataset.append((qsnan[0],"Signaling NaN"))
        dataset.append((qqnan[0],"Quiet NaN"))

        dataset.append((qzero[1],"-0"))
        dataset.append((floatingPoint_tohex(128,float(random.uniform(-1,0))),"A random number in the range (-1, -0)"))
        dataset.append((qone[1],"-1"))
        for i in range(-275,-100,25):
            dataset.append((floatingPoint_tohex(128, i/100),"Number = "+str(i/100)+" => Number ∈ [-2.75,-1)"))
        dataset.append((floatingPoint_tohex(128,float(random.uniform(-2**127-1,-1))),"A random number in the range (-1.11..11*2^precision, -1)"))
        dataset.append((floatingPoint_tohex(128,float(-2**127-1)),"-MaxInt"))
        dataset.append((qinfinity[1],"-Infinity"))
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B28 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
...
def ibm_b29(flen, iflen, opcode, ops, seed=10):
    ...
    if iflen == 32:
        ...
        for sgn in sgns:
            for i in range(8):
                LeastGuardSticky = '{:03b}'.format(i)
                hexnum = "0x" + hex(int("1"+sgn + "011111111111100" + mant + LeastGuardSticky,2))[3:]
                dataset.append((hexnum,"Exp = -3; Sign = {}; LSB = {}; Guard = {}; Sticky = {}"\
                    .format(sgn,LeastGuardSticky[0],LeastGuardSticky[1],LeastGuardSticky[2])))
    ...
    mess='Generated'+ (' '*(5-len(str(len(coverpoints)))))+ str(len(coverpoints)) +' '+\
    (str(32) if iflen == 32 else str(64) if iflen == 64 else str(128)) + '-bit coverpoints using Model B29 for '+opcode+' !'
    logger.debug(mess)
    coverpoints = comments_parser(coverpoints)
